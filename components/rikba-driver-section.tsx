...
export function RikbaDriverSection() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("https://formspree.io/f/xwvnyjbr", {
        method: "POST",
        body: formData,
    ...
    <section id="drivers" className="py-20 md:py-28 scroll-mt-20" style={{ background: "var(--brand-navy)" }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-14 lg:gap-20 items-center">
          {/* Left: Image + comparison card */}
          <div className="relative">
            <div className="rounded-3xl overflow-hidden shadow-2xl">
              <img src="https://i.imgur.com/LxQJn0T_d.webp?maxwidth=760&fidelity=grand" alt="Rikba driver in Malta" className="w-full h-72 md:h-96 object-cover" />
              <div className="absolute inset-0 rounded-3xl" style={{ background: "linear-gradient(to top, rgba(0,26,64,0.7) 0%, transparent 60%)" }} />
            </div>

            <div className="absolute bottom-4 left-4 right-4 md:bottom-6 md:left-6 md:right-6 rounded-2xl p-5 shadow-xl progress-bars-container" style={{ background: "rgba(255,255,255,0.97)", backdropFilter: "blur(12px)" }}>
              <p className="text-xs font-bold uppercase tracking-widest mb-4" style={{ fontFamily: "DM Sans, sans-serif", color: "var(--text-secondary)" }}>
                ...
                {[
                  ...
                  <div key={row.name} className={`progress-bars ${index === 1 ? "rikba-fill" : "other-fill"}`} data-width={row.pct}>
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-xs font-semibold" style={{ fontFamily: "DM Sans, sans-serif", color: "var(--text-primary)" }}>{row.name}</span>
                      <span className="text-xs font-bold" style={{ color: row.bad ? "#ef4444" : "#0066ff" }}>{row.label}</span>
                    </div>
              ...
              <p className="text-xs mt-3 italic" style={{ fontFamily: "DM Sans, sans-serif", color: "var(--text-secondary)" }}>
          ...
          <div className="flex flex-col items-center">
            <div className="text-center mb-8">
              <span className="inline-block text-xs font-bold uppercase tracking-widest mb-3 px-3 py-1 rounded-full" style={{ color: "var(--brand)", background: "rgba(0,102,255,0.15)", fontFamily: "DM Sans, sans-serif" }}>
                For Drivers
              </span>
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold leading-tight mb-5 text-white" style={{ fontFamily: "Syne, sans-serif" }}>
                Drive in Malta.<br />
                <span style={{ color: "var(--brand)" }}>Earn More.</span>
              </h2>
            </div>

            <p className="text-base md:text-lg leading-relaxed mb-10 text-center max-w-lg" style={{ fontFamily: "DM Sans, sans-serif", color: "rgba(255,255,255,0.7)" }}>
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-8 w-full max-w-2xl">
              {perks.map((p, i) => (
                <div key={i} className="rounded-xl p-4 flex gap-3 items-start transition-all duration-200 hover:scale-105" style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)" }}>
                  <span className="text-2xl mt-0.5 shrink-0">{p.icon}</span>
                  <div>
                    <p className="text-sm font-bold text-white mb-1" style={{ fontFamily: "Syne, sans-serif" }}>{p.label}</p>
                    <p className="text-xs leading-relaxed" style={{ fontFamily: "DM Sans, sans-serif", color: "rgba(255,255,255,0.6)" }}>{p.desc}</p>
                  </div>
                </div>
              ))}
            </div>
